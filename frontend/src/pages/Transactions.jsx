...
const Transactions = () => {
  ...
  const allCategories = ["Food", "Transport", "Entertainment", "Shopping", "Bills", "Healthcare", "Salary", "Freelance", "Investment", "Gift", "Travel", "Other"];
  ...
  const filteredTransactions = useMemo(() => {
    return transactions.filter(transaction => {
      ...
      // Mode filter
      if (filters.mode !== "all" && transaction.mode !== filters.mode) {
        return false;
      }

      // Payee filter
      if (filters.payee !== "all" && transaction.payee !== filters.payee) {
        return false;
      }
  ...
  const analytics = useMemo(() => {
    ...
    filteredTransactions.forEach(t => {
      if (!payeeStats[t.payee]) {
        payeeStats[t.payee] = { income: 0, expense: 0, count: 0 };
      }
      if (t.type === "income") {
        payeeStats[t.payee].income += Math.abs(t.amount);
      } else {
        payeeStats[t.payee].expense += Math.abs(t.amount);
      }
